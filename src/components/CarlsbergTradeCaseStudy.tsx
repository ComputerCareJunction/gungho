...
export default function CarlsbergTradeCaseStudy() {
  return (
    <section
      className="relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-lg ring-1 ring-slate-900/5"
      aria-labelledby="carlsberg-trade-heading"
    >
      <div
        className="absolute inset-0 bg-[radial-gradient(ellipse_120%_80%_at_50%_-20%,rgba(249,115,22,0.15),transparent_55%),linear-gradient(180deg,#fff7ed_0%,#ffffff_45%,#f8fafc_100%)]"
        aria-hidden
      />
      <div className="relative px-5 py-12 sm:px-8 sm:py-14 md:px-10 md:py-16">
        <header className="mx-auto w-full text-center">
          ...
          <p className="mx-auto mt-5 max-w-3xl text-pretty text-sm leading-relaxed text-slate-600 sm:mt-6 sm:text-base md:text-lg">
            {cs.heroIntro}
          </p>
          ...
          <div className="mx-auto mt-8 max-w-3xl space-y-6 text-left sm:mt-10">
            <div>
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-700 sm:text-base">{ta.carlsberg.clientBrief}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-primary sm:text-sm">
                {ta.carlsberg.solutionHeading}
              </p>
              <p className="mt-2 text-sm leading-relaxed text-slate-700 sm:text-base">{ta.carlsberg.solution}</p>
            </div>
          </div>
        </header>
        ...
        >
          {carlsbergImages.map((src, i) => (
            <li key={src} className="min-w-0">
              <div className="group/img relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-md ring-1 ring-slate-900/5 transition-shadow duration-300 hover:bg-slate-100 hover:shadow-primary/15">
        ...
        <div className="mx-auto mt-10 grid w-full grid-cols-1 gap-5 sm:mt-12 sm:grid-cols-3 sm:gap-6 md:mt-14">
          {cs.featureCards.map((card, index) => {
            ...
            return (
              <div
                key={card.title}
                className="rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-md transition-colors duration-300 hover:border-primary/30 hover:bg-slate-100 hover:shadow-lg"
              >
                ...
                <p className="mt-2 text-sm leading-relaxed text-slate-600 sm:text-[15px]">{card.description}</p>
              </div>
            );
          })}
        </div>
